...
import { decodeEventLog, decodeFunctionData, encodeFunctionData, formatEther, formatUnits, getAddress, http, parseAbiItem } from "viem";
import { createPublicClient } from "viem";
import { erc20Abi, positionManagerAbi, transferEvent } from "./abis.js";
import { discoverCandidatePools, getFullCandidateUniverse } from "./candidate.js";
import { DEADLINE_SECONDS, NATIVE_OKB_SENTINEL, SLIPPAGE_BPS, UNISWAP_V3_FACTORY, UNISWAP_V3_POSITION_MANAGER, WOKB_ADDRESS, xlLayerChain } from "./config.js";
import { parseIntent } from "./intent.js";
import { DEFAULT_SWAP_PLANNING_MODE, OFFICIAL_UNISWAP_SKILL_NAMES } from "./official-skills.js";
import { buildSwapApproval, buildSwapTx, executeContractCall, getWalletBalance, scanRequiresExplicitWarning, scanRequiresStop, scanTransaction } from "./onchainos.js";
import { buildFundingPlan, classifyRangeStatus, deriveRangePlan, formatPairSymbol, tokenIsStable, withApprovalBuffer } from "./strategy.js";
...
function parsePairSelection(candidates, intent) {
    ...
    const pairVariants = new Set([intent.pairHint]
...
async function enrichFundingPlan(plan) {
    if (!plan.selectedPool || !plan.targetAmount0 || !plan.targetAmount1) {
        return plan;
    }
    try {
        const inventory = await getInventory(plan.selectedPool);
        plan.fundingPlan = buildFundingPlan(plan.selectedPool, inventory, plan.targetAmount0, plan.targetAmount1, plan.swapPlanningMode);
...
export async function planLp(intentInput, options = {}) {
...
async function getInventory(pool) {
    ...
    const walletAddress = getAddress(wallet.evmAddress.toLowerCase());
...
async function getCurrentWalletAddress() {
    const wallet = await getWalletBalance();
    return getAddress(wallet.evmAddress.toLowerCase());
}
function createSdkPool(pool) {
    ...
    return new Pool(token0, token1, pool.fee, pool.sqrtPriceX96.toString(), pool.liquidity.toString(), pool.currentTick);
}
...
async function executeWrap(walletAddress, amount) {
...
async function executeApprovalIfNeeded(params) {
    ...
    const approval = params.mode === "swap-helper"
...
function parseApprovalSpender(data) {
    try {
...
function getDesiredMintAmounts(inventory, pool, targetAmount0, targetAmount1) {
    ...
    if (available0 < targetAmount0 || available1 < targetAmount1) {
        throw new Error(`Wallet inventory is still short after funding actions for ${formatPairSymbol(pool.token0, pool.token1)}.`);
    }
    return {
        amount0: available0,
        amount1: available1
...
export async function openLp(intentInput, confirmOrOptions) {
    ...
    const inventory = await getInventory(plan.selectedPool);
    const fundingPlan = buildFundingPlan(plan.selectedPool, inventory, plan.targetAmount0, plan.targetAmount1, plan.swapPlanningMode);
    ...
    const approvals = [];
    const steps = [];
    let swapExecutionSource = "not-needed";
    ...
    const desiredMint = getDesiredMintAmounts(refreshedInventory, plan.selectedPool, plan.targetAmount0, plan.targetAmount1);
...
async function getPositionPool(position) {
...
export async function monitorPosition(tokenIdInput) {
    ...
    return {
        ...
        recommendation: rangeStatus === "out_of_range"
...
export async function planCloseLp(tokenIdInput) {
    const tokenId = BigInt(tokenIdInput);
...
export async function closeLp(tokenIdInput, confirm = false) {
...
export async function suggestReposition(tokenId) {
    ...
    return {
        ...
        recommendation: health.rangeStatus === "in_range"
