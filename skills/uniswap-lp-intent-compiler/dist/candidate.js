...
import { BASE_ANALYTICS_SOURCES, DEFAULT_PLAN_CANDIDATE_LIMIT, DISCOVERY_SYMBOL_QUERIES, SWAP_ACTIVITY_LOOKBACK_BLOCKS, TICK_SPACING_BY_FEE, UNISWAP_V3_FACTORY, WOKB_ADDRESS, xlLayerChain } from "./config.js";
import { parseIntent } from "./intent.js";
import { getMarketKlines, searchTokenByAddress, searchTokenQuery } from "./onchainos.js";
import { calculateRealizedVolatility, classifyPairArchetype, getFeeGuidance, getRecommendedFeeTier, formatPairSymbol, priceFromSqrtRatioX96, scoreCandidate, tokenIsBluechip, tokenIsStable } from "./strategy.js";
...
function normalisePairHint(pairHint) {
    if (!pairHint) {
        return undefined;
    }
...
async function getTokenMetadata(address) {
    ...
    const promise = (async () => {
        ...
        return {
            address: getAddress(item.tokenContractAddress.toLowerCase()),
...
async function getTokenVolatility(address) {
    const key = address.toLowerCase();
...
async function getRecentSwapCount(poolAddress) {
...
async function getDiscoveryUniverse(intent) {
    ...
    const promise = (async () => {
        ...
        for (const resultSet of queryResults) {
            for (const item of resultSet) {
                ...
                const address = getAddress(item.tokenContractAddress.toLowerCase());
...
async function getOfficialAnalytics(poolAddress, token0, token1) {
...
async function buildPoolState(poolAddress) {
    ...
    const token0 = await getTokenMetadata(token0Address);
    const token1 = await getTokenMetadata(token1Address);
...
export async function discoverCandidatePools(intentInput) {
    ...
    for (const address of poolAddresses) {
        try {
            const state = await buildPoolState(getAddress(address));
            if (state) {
...
export async function getFullCandidateUniverse() {
