...
export const REQUIRED_OKX_ENV_KEYS = ["OKX_API_KEY", "OKX_SECRET_KEY", "OKX_PASSPHRASE"];
export const REQUIRED_UNISWAP_ENV_KEYS = ["UNISWAP_API_KEY"];
...
export function deriveExecutionMode(facts) {
    return facts.uniswapApiKeyReady ? "official-skills-hybrid" : "fallback-onchainos";
}
export function derivePreflightSummary(facts) {
    const ready = facts.officialSkillsReady &&
...
export async function getPreflightStatus() {
    ...
    if (walletStatus.ok) {
        try {
            const parsed = JSON.parse(walletStatus.stdout || walletStatus.stderr);
        ...
        checks: [
            {
                ...
                remediation: "Install liquidity-planner and swap-integration into $CODEX_HOME/skills/uniswap/ before running prize mode."
            },
            {
                id: "onchainos-cli",
