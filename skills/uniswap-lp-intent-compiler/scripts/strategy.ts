...
const { nearestUsableTick } = require("@uniswap/v3-sdk") as typeof import("@uniswap/v3-sdk");
...
import {
  ...
  NATIVE_OKB_SENTINEL,
  OFFICIAL_RANGE_WIDTHS,
  PLAN_CONFIRMATION_TOLERANCE,
  RECOMMENDED_FEE_TIER_BY_ARCHETYPE,
  STABLE_SYMBOLS,
  WOKB_ADDRESS,
  WRAPPED_SYMBOL_PREFIXES
} from "./config.js";
import type {
  CandidatePool,
  FundingAction,
...
function sameCorrelatedGroup(left: string, right: string): boolean {
  const leftUpper = left.toUpperCase();
  const rightUpper = right.toUpperCase();
  return CORRELATED_SYMBOL_GROUPS.some(
    (group) => (group as readonly string[]).includes(leftUpper) && (group as readonly string[]).includes(rightUpper)
  );
}

function feeTierScore(poolFee: number, archetype: PairArchetype): number {
...
function describeFeeTier(archetype: PairArchetype, fee: number): string {
  const recommended = RECOMMENDED_FEE_TIER_BY_ARCHETYPE[archetype];
  if (fee === recommended) {
...
function getSwapSourceForMode(mode: SwapPlanningMode): "uniswap-trading-api" | "onchainos-dex" {
  return mode === "official-skills-hybrid" ? "uniswap-trading-api" : "onchainos-dex";
}

export function formatPairSymbol(token0: TokenMetadata, token1: TokenMetadata): string {
  return `${token0.symbol}/${token1.symbol}`;
}

export function tokenIsStable(token: TokenMetadata): boolean {
  return STABLE_SYMBOLS.has(token.symbol.toUpperCase()) || (token.priceUsd > 0.98 && token.priceUsd < 1.02);
}

export function tokenIsBluechip(token: TokenMetadata): boolean {
  return BLUECHIP_SYMBOLS.has(token.symbol.toUpperCase());
}

export function calculateRealizedVolatility(closes: number[]): number {
  if (closes.length < 3) {
    return 0;
  }
...
export function classifyPairArchetype(token0: TokenMetadata, token1: TokenMetadata): {
...
export function getRecommendedFeeTier(archetype: PairArchetype): number {
  return RECOMMENDED_FEE_TIER_BY_ARCHETYPE[archetype];
}

export function getFeeGuidance(archetype: PairArchetype, liveFeeTier: number): string {
  return describeFeeTier(archetype, liveFeeTier);
}

export function getOfficialRangeWidth(archetype: PairArchetype, profile: RiskProfile): number {
  return OFFICIAL_RANGE_WIDTHS[archetype][profile];
}

export function scoreCandidate(pool: PoolState, preference: IntentPreference): CandidatePool {
  ...
  const liquidityUniverseUsd = Math.max(
    pool.token0.liquidityUsd + pool.token1.liquidityUsd,
    dexLiquidityUsd,
    defillamaTvlUsd
  );
    ...
    pool.pairArchetype === "stable"
      ...
      : pool.pairArchetype === "correlated"
        ...
        : pool.pairArchetype === "major"
          ...
          : pool.volatility24h / 0.14,
    0,
    1
  );
  const priceShockPenalty = clamp(
    (Math.abs(pool.token0.change24hPct) + Math.abs(pool.token1.change24hPct)) /
      (pool.pairArchetype === "stable" ? 8 : pool.pairArchetype === "correlated" ? 18 : 40),
    0,
    1
  );
  const recognitionPenalty =
    (pool.token0.communityRecognized ? 0 : 0.2) + (pool.token1.communityRecognized ? 0 : 0.2);
  ...
  const preferenceBonus =
    preference === "stable" && pool.pairArchetype === "stable"
      ? 0.08
  ...
  const yieldScore = Math.round(
    100 * clamp(0.3 * recentActivityScore + 0.25 * volumeScore + 0.2 * feeScore + 0.15 * liquidityScore + 0.1 * apyScore, 0, 1)
  );
  const riskScore = Math.round(
    100 *
      clamp(
        0.4 * volatilityPenalty +
  ...
  const overallScore = Math.round(
    clamp(
      yieldScore - riskScore * 0.5 + 100 * (stableBonus + correlatedBonus + majorBonus + preferenceBonus),
      0,
      100
    )
  );
...
export function priceToTick(priceToken1PerToken0: number, token0Decimals: number, token1Decimals: number): number {
...
export function priceFromSqrtRatioX96(
  sqrtPriceX96: bigint,
...
export function deriveRangePlan(pool: PoolState, profile: RiskProfile): RangePlan {
...
function tokenAddressEquals(left: string, right: string): boolean {
  return left.toLowerCase() === right.toLowerCase();
}

function toHumanAmount(amount: bigint, decimals: number): number {
  return Number(amount) / 10 ** decimals;
}

function toRawAmount(amountHuman: number, decimals: number): bigint {
  return BigInt(Math.floor(amountHuman * 10 ** decimals));
}

export function withApprovalBuffer(amount: bigint): bigint {
  return (amount * BigInt(10_000 + APPROVAL_BUFFER_BPS)) / 10_000n;
}
...
export function classifyRangeStatus(currentTick: number, lowerTick: number, upperTick: number): PositionHealth["rangeStatus"] {
