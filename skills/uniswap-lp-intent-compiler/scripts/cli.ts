...
function parseMode(input: string | boolean | undefined): SwapPlanningMode | undefined {
...
function intentFromOptions(options: Record<string, string | boolean>) {
  ...
  if (typeof options.pool === "string") {
    parsed.poolAddressHint = String(options.pool).toLowerCase() as `0x${string}`;
  }
  return parsed;
}
